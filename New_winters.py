...
class Holt_winters:
    ...
    def initialise_components(self):
        ...
        initial_slope = mean(slopeslist)

        #seasonality needs to be defined for a season's worth of data - using additive not multiplicative
        initial_seasonalcomponents = []
        grand_mean = mean(self.past_data)
        for i in range(self.season_length):
            initial_seasonalcomponents.append(initialisation_data[i]-grand_mean)
    ...
    def smooth_past_data(self):
        ...
        current_seasonal_components = self.initial_seasonalcomponents
        
        seasonal_index = 0

        for t in range(len(self.past_data)):
            
            
            seasonal_index += 1
            seasonal_index = seasonal_index % self.season_length
            ...
            self.forecast_data.append(0)

        self.previous_level = previous_level
        self.previous_trend = previous_trend
        self.seasonal_index = seasonal_index
        self.seasonal_components = current_seasonal_components
        ### forecast ###

    def forecast(self,forecast_length = 120):
        self.smooth_past_data()

        previous_level = self.previous_level
        previous_trend = self.previous_trend
        current_seasonal_components = self.seasonal_components
        ...
        for t in range(forecast_length):
            ...
            self.forecast_data.append(previous_level + previous_trend + previous_seasonal_component)

            y = self.forecast_data[-1]
...
Test = Holt_winters(temperatures[:2*(len(temperatures)//3)], 120, 24)
Test.create_coefficients()
Test.forecast(Test.forecast_length)
print(f"Smoothed data points: {len(Test.smoothed_data)}")


plt.figure(figsize=(14, 7))
plt.plot(temperatures, label='Original Data', color='blue')
plt.plot(Test.forecast_data, label='Forecast Data', color='red', linestyle='--')
